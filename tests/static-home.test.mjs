...
test("static packaging rejects missing or incomplete output and clears stale homepage", async () => {
  ...
  try {
    ...
    await writeFile(new URL("dist/server/vinext-prerender.json", root), JSON.stringify({ routes: [{ route: "/", status: "rendered", revalidate: false }] }));
    for (const html of [null, "<html>Your watchlist Connecting to ESPN</html>"]) {
