...
import { ACC, gameExpectation, teamRank } from "./upset";
import { isPowerFour } from "./conference-evidence";
...
export function teamRelevance(game: Game) {
  ...
  return Math.min(50, (acc ? 20 : 0) + (acc === 2 ? 2 : 0)
    + (game.teams.some(t => t.conferenceId !== ACC && isPowerFour(t)) ? 4 : 0)
...
export function gamePriority(game: Game) {
  ...
  let upset = 0, disruption = 0;
  // Ranking interest is independent of who the betting line favored. Preserve
  // tie/narrow-lead retention without inventing an upset label or notification.
  const ranked = [...game.teams].sort((a, b) => (teamRank(a) ?? Infinity) - (teamRank(b) ?? Infinity));
  const [higher, lower] = ranked;
  const higherRank = teamRank(higher), lowerRank = teamRank(lower);
  if (stage && higherRank !== null && (lowerRank !== null ? lowerRank > higherRank : lower.rankKnown === true)
    && typeof higher.score === "number" && Number.isFinite(higher.score)
    && typeof lower.score === "number" && Number.isFinite(lower.score)) {
    const difference = higher.score - lower.score;
    const stateWeight = difference < 0 ? 1 : difference === 0 ? 0.75 : difference <= 3 && stage >= 5 ? 0.5 : 0;
    disruption = 35 * (1 - comfort) * stateWeight * stageWeight * marginWeight;
  }
  ...
  return { total: relevance + drama + upset + disruption, relevance, drama, upset, disruption, stage, finishBand };
}
