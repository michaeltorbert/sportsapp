...
export function createAppUpdater(loadedValue: string | undefined, publish: (state: UpdateState) => void, navigate: (commit: string) => void) {
  ...
  const request = () => {
    ...
    const c = new AbortController(); controller = c; last = Date.now();
  ...
  function accept(result: NonNullable<ReturnType<typeof metadata>>, manual: boolean) {
    ...
    if (!candidate) { confirmed = null; emit({ target: null, status: "The app is up to date." }); return; }
    if (candidate.confirmed) {
      ...
      emit({ target: dismissed.has(confirmed) ? null : confirmed, status: "An app update is available." });
    } else {
      emit({ status: "An app update is being verified." });
  ...
  async function check(manual = false) {
    ...
    if (manual) { manualPending = true; emit({ status: "Checking for an app update…" }); }
    ...
    else if (requested) emit({ status: "Unable to check for an app update. Please try again." });
  }
  async function refresh() {
    if (!alive || busy || !loaded) return;
    busy = true; clearTimeout(scheduled); scheduled = undefined; clearTimeout(confirmation); confirmation = undefined;
    ...
    try {
      ...
      if (result.commit === loaded) accept(result, false);
      else { navigate(result.commit); return; }
  ...
  return {
    ...
    dismiss() { if (!confirmed || busy) return; dismissed.add(confirmed); manualCandidate = ""; try { sessionStorage.setItem("ss:app-update-dismissed", JSON.stringify([...dismissed])); } catch { /* Optional. */ } emit({ target: null }); },
    dispose() { alive = false; controller?.abort(); clearTimeout(initial); clearInterval(interval); clearTimeout(scheduled); clearTimeout(confirmation); window.removeEventListener("focus", resume); window.removeEventListener("online", resume); window.removeEventListener("pageshow", pageshow); document.removeEventListener("visibilitychange", resume); },
  };
}
