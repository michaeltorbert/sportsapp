import { parsePregameLine, classifyPregameEvidence } from "./pregame-lines";
import { z } from "zod";
...
export function broadcastNames(raw: unknown, fallback: unknown): string[] {
  const values = Array.isArray(raw) ? raw.flatMap(item => item && typeof item === "object" && Array.isArray(item.names) ? item.names : []) : [];
  ...
  return names.length ? names : typeof fallback === "string" && fallback.trim() ? [fallback.trim()] : [];
}
...
export function normalizeScoreboard(raw: unknown, date: string, fetchedAt = new Date().toISOString(), endDate = date): Scoreboard {
  ...
  for (const item of envelope.events) {
    ...
    const broadcasts = broadcastNames(c.broadcasts, c.broadcast);
    games.set(e.id, { id: e.id, date: gameDate, timeValid: c.timeValid !== false, neutralSite: typeof c.neutralSite === "boolean" ? c.neutralSite : undefined, state, started, pregameLine: !started && (state === "upcoming" || state === "delayed") ? parsePregameLine(c.odds, teams[0].id, teams[1].id) : undefined, clockKnown, intermission, halftime: statusName === "STATUS_HALFTIME", status: s.type.shortDetail || s.type.detail || s.type.description || "Status unavailable", period: s.period || 0, clock: s.clock || 0, teams, broadcasts, broadcast: broadcasts.join(" / "), possession: c.situation?.possession === undefined ? null : String(c.situation.possession), downDistance: c.situation?.downDistanceText || "", redZone: c.situation?.isRedZone === true, url: espnLink(e.links?.find(l => l.rel?.includes("summary"))?.href, `https://www.espn.com/college-football/game/_/gameId/${encodeURIComponent(e.id)}`) });
