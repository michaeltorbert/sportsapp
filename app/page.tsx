...
function GameCard({ game, showDate = false, expanded, onExpanded }: { game: Game; showDate?: boolean; expanded: boolean; onExpanded: (open: boolean) => void }) {
...
export default function Home() {
  ...
  const section = (title: string, state: Game["state"]) => {
    ...
    return list.length > 0 && <section className="score-section"><div className="section-label"><h2>{state === "live" && <span className="live-dot" />}{title}</h2><span>{list.length} {list.length === 1 ? "game" : "games"}</span></div><div className="game-grid">{list.map(g => <GameCard key={g.id} game={g} showDate={weekly} expanded={!!expandedGames[g.id]} onExpanded={open => setExpandedGames(previous => ({ ...previous, [g.id]: open }))} />)}</div></section>;
  };
