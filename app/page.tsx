...
function GameCard({ game, showDate = false }: { game: Game; showDate?: boolean }) {
  ...
  return <article id={`game-${game.id}`} className={`game-card ${tags.upset ? "upset-card" : ""} ${game.state === "final" ? "final-card" : ""}`}>
    ...
    {result.upset && <div className="upset-reason"><Zap size={14} fill="currentColor" /><span>{leader.name} {game.state === "final" ? "beat" : "leads"} No. {trailing.rank} {trailing.name}</span></div>}
...
function Help() { return <Sheet><SheetTrigger asChild><button className="icon-button" aria-label="How this scoreboard works"><CircleHelp size={21} /></button></SheetTrigger><SheetContent side="bottom" className="help-sheet"><SheetHeader><SheetTitle>How your watchlist works</SheetTitle><SheetDescription>The right games, automatically.</SheetDescription></SheetHeader><div className="help-body"><dl><div><dt>ACC</dt><dd>Either team belongs to the ACC. The ACC tab covers the current football week, Thursday through Monday.</dd></div><div><dt>Top 25</dt><dd>Either team has an ESPN curated ranking from 1 through 25.</dd></div><div><dt>One score</dt><dd>A live FBS game, in any quarter or at halftime, tied or within 8 points.</dd></div><div><dt>Upset watch</dt><dd>A ranked team trails an unranked or lower-ranked opponent. Ties do not qualify. Rankings come from ESPN, not betting odds.</dd></div></dl><p>The Watchlist combines all four categories without repeating games. Live games appear first: ACC, then Top 25, then the rest. Within each group: latest quarter, closest score, least time left.</p><p>Finals keep their last observed category and any category matching the final score for that game day. Hide finals remembers your choice on this device.</p><p>Today stays on the previous Eastern day while its games are unfinished, including games continuing past midnight. It moves forward after the last game finishes. A date you pick manually stays selected.</p><p>Scores refresh every 30 seconds while the app is open. ESPN clocks may lag the broadcast. Failed refreshes keep the last displayed scores with a warning.</p><div className="install-tip"><h3>Keep it on your iPhone</h3><p>In Safari, tap Share, then Add to Home Screen. Keep Open as Web App enabled if offered. Open that icon to set up alerts.</p></div><h3 className="history-heading">Version history</h3><ol className="release-history">{releases.map(r => <li key={r.version}><strong>v{r.version}</strong><span>{r.date}</span><p>{r.changes}</p></li>)}</ol><a href="https://www.espn.com/college-football/scoreboard" target="_blank" rel="noopener noreferrer" className="source-link">ESPN college football scoreboard <ArrowUpRight size={15} /></a></div></SheetContent></Sheet>; }
