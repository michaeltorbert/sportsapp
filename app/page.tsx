...
function Help() { return <Sheet><SheetTrigger asChild><button className="icon-button" aria-label="How this scoreboard works"><CircleHelp size={21} /></button></SheetTrigger><SheetContent side="bottom" className="help-sheet"><SheetHeader><SheetTitle>How your watchlist works</SheetTitle><SheetDescription>The right games, automatically.</SheetDescription></SheetHeader><div className="help-body"><dl><div><dt>ACC</dt><dd>Either team belongs to the ACC. The ACC tab covers the current football week, Thursday through Monday.</dd></div><div><dt>Top 25</dt><dd>Either team has an ESPN curated ranking from 1 through 25.</dd></div><div><dt>One score</dt><dd>A live FBS game, in any quarter or at halftime, tied or within 8 points.</dd></div><div><dt>Upset watch</dt><dd>A pregame favorite trails in a ranked matchup, or an ACC/SEC favorite trails even when unranked. Available ESPN pregame lines take precedence over rankings. Without a line, we use known rankings; unranked ACC/SEC teams trailing American, C-USA, MAC, Mountain West, or Sun Belt opponents appear as a labeled conference watch. Conference membership does not prove who was favored. Ties get close-game attention, not a trailing label. Unranked watches appear in the list; upset push notifications currently cover ranked teams.</dd></div></dl><p>The Watchlist combines all four categories without repeating games. Live games combine team relevance, live drama, and upset significance. ACC interest and rankings add together; close fourth quarters, the final five minutes, and overtime rise. Major upsets can outrank ACC blowouts. Top 10 close finishes outrank comparable unranked ACC games. Early ties get little urgency, and large late margins reduce priority. Meaningful time bands keep the order steady. Upcoming games remain chronological.</p><p>Finals keep their last observed category and any category matching the final score for that game day. Hide finals remembers your choice on this device.</p><p>Today stays on the previous Eastern day while its games are unfinished, including games continuing past midnight. It moves forward after the last game finishes. A date you pick manually stays selected.</p><p>Scores refresh every 30 seconds while the app is open. ESPN clocks may lag the broadcast. Failed refreshes keep the last displayed scores with a warning.</p><div className="install-tip"><h3>Keep it on your iPhone</h3><p>In Safari, tap Share, then Add to Home Screen. Keep Open as Web App enabled if offered. Open that icon to set up alerts.</p></div><h3 className="history-heading">Version history</h3><ol className="release-history">{releases.map(r => <li key={r.version}><strong>v{r.version}</strong><span>{r.date}</span><p>{r.changes}</p></li>)}</ol><a href="https://www.espn.com/college-football/scoreboard" target="_blank" rel="noopener noreferrer" className="source-link">ESPN college football scoreboard <ArrowUpRight size={15} /></a></div></SheetContent></Sheet>; }
...
export default function Home() {
  ...
  useEffect(() => {
    const target = /^#game-([A-Za-z0-9_-]+)$/.exec(location.hash)?.[1] || "";
