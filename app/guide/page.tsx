...
export default function Guide() {
  ...
  const duke = useDukeVisibility(data?.games || []);
  const protectedBoard = useMemo(() => protectDukeBoard(data, duke.prefs), [data, duke.prefs]);
  ...
  return <main className="app-shell guide-shell" data-app-commit={LOADED_COMMIT}>
    <header className="app-header"><Link className="wordmark" href="/" aria-label="Saturday Signal home"><span className="brand-icon"><Signal size={22} /></span><span>saturday<span className="brand-light">signal</span></span></Link><div className="header-actions"><button className={`icon-button ${refreshing ? "refreshing" : ""}`} aria-label="Refresh guide" onClick={refresh} disabled={refreshing || !date}><RefreshCw size={20} /></button><DukeSettings games={data?.games || []} visibility={duke} /></div></header>
    ...
    <div className="guide-heading"><h1>Guide</h1><DukeGameControls games={data?.games || []} visibility={duke} /><div className="guide-modes" role="group" aria-label="Guide games"><button aria-pressed={selection.view === "all"} onClick={() => selection.choose({ date: selection.date, view: "all" })}>All games</button><button aria-pressed={selection.view === "watch"} onClick={() => selection.choose({ date: selection.date, view: "watch" })}>Watchlist only</button></div></div>
    ...
    {protectedBoard && <GuideTimeline board={protectedBoard} mode={selection.view} now={now} followToday={followToday} />}
