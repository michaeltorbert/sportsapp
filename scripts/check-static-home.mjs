...
const temporary = await mkdtemp(path.join(root, "dist/server/static-check-"));
let child;
try {
  ...
  await writeFile(path.join(temporary, "guard.mjs"), `import worker from "../index.js"; export default { fetch(request, env, ctx) { if (new URL(request.url).pathname === "/") return new Response("ROOT_REACHED_WORKER", { status: 599 }); return worker.fetch(request, env, ctx); } };`);
  const socket = createServer();
  socket.listen(0, "127.0.0.1");
  await once(socket, "listening");
  const { port } = socket.address();
  await new Promise(resolve => socket.close(resolve));
  child = spawn(process.execPath, ["node_modules/wrangler/bin/wrangler.js", "dev", "--config", path.join(temporary, "wrangler.json"), "--local", "--ip", "127.0.0.1", "--port", String(port), "--inspector-port", "0"], { cwd: root, detached: true, stdio: ["ignore", "pipe", "pipe"], env: { ...process.env, WRANGLER_SEND_METRICS: "false", WRANGLER_LOG_PATH: ".wrangler/logs", BROWSER: "none" } });
  let output = "";
