...
Once the credential is configured, run **Cloudflare preview** from GitHub Actions and select **main** in the **Run workflow** branch selector, after the intended changes have been reviewed and merged. Record the intended reviewed commit before dispatch. After dispatch, record the workflow run URL, its authoritative `head_sha`, and the version/commit reported by preview `/api/health`; the run head and health commit must equal the intended reviewed commit. If `main` advanced during dispatch, stop acceptance and review the actual run commit before deciding whether to dispatch again. A successful run for another commit does not verify the intended commit. The workflow checks out its own run ref; there is no separate ref input that could bypass the environment policy. It builds/tests that source, deploys only the preview website, and checks its exact commit, page, manifest, service worker, and current scores. It never deploys the alert Worker, writes subscription records, or sends notifications. Preview is a public provider address, not a private review link; do not put private data there. After a successful run, check daily/ACC/Top 25 switching and score refresh in the browser, plus the manifest, service worker, and referenced app assets. Before calling preview verified, retain a required before/after comparison of production website `/api/health` version and commit, production website deployment identity, alert Worker deployment identity, and alert Worker configuration, including both allowed-origin `/config` responses. Require unchanged deployment versions and configuration; readiness timestamps may advance through normal scheduled polling. Confirm the preview run performed no subscription or delivery mutations; do not confuse ordinary production cron activity with preview writes. If any deployment identity or configuration differs, investigate and leave preview verification incomplete until the change is explained. Keep the exact alert-origin allowlist; do not add wildcard preview origins. The preview origin is not allowed by the alert service, so alert setup being unavailable there is expected. Do not subscribe or invoke notification tests from preview. A missing credential or failed verification leaves the preview setup incomplete, even if the build or deployment step passed.
