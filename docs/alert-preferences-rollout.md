...
The preflight requires `preferencesVersion=1` and `preferencesReady=true`; the latter means VAPID configuration is present, the delivery gate is enabled, and a positive baseline epoch exists. It reads `public/alerts-config.json` and makes a Node GET without a browser Origin header. Overall `ready` and future unrelated readiness reasons do not control this prerequisite.
