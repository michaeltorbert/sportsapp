...
Check the website's `/api/health` for exact version and source commit. Check the alerts Worker's `/config` separately with each exact production Origin header; keep its `ready`, `readinessReason`, `lastTickAt`, `lastSuccessfulPollAt`, version and CORS result. Readiness is not proof of phone receipt. `last_tick` indicates scheduler activity, while `last_good_score` indicates a successful poll under the current parser's coverage rules. Since [#18](https://github.com/michaeltorbert/sportsapp/issues/18), a CDN board counts only when its calendar proves yesterday and today are covered; otherwise the poller tries the date-specific API, and a poll where both fail leaves `last_good_score` unchanged. Once the last successful poll is at least twenty minutes old, `/config` reports `stale-score-feed` if the earlier VAPID configuration, scheduler tick, and prior-success checks pass. On the selected calendar entry's boundary days the CDN is rejected by design, so a stale feed there with the site API blocked is a real failed poll, not a scheduler fault. Freshness alone still does not prove every expected game was present.
