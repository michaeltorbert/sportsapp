...
## Close-game slate prioritization (#82)

Only delivery of `one-score-fourth` changes: a fresh event between two known-unranked Group-of-Six teams can be skipped when another non-Duke current live game has a fresh, currently satisfied `one-score-fourth` or `ranked-trailing-fourth` event and involves a ranked or Power-Four team, or a supported meaningful upset. Power Four IDs are 1/4/5/8; Group of Six IDs are 151/12/15/17/9/37, including Pac-12. Unknown ranks, unknown conferences, independents and FCS are not presumed two-unranked Group of Six. Kickoffs and finals never suppress live alerts.

The gate is computed once from the fresh-event/current-game snapshot before recipient processing. It is global, including stronger triggers a recipient disabled or already received, not recipient-deliverable competition. Scoreboard categories and ordering never select notifications. Skipped events make no claim and consume no sibling allowance. They retain only their original three-minute window: no new ID, refreshed timestamp, baseline reset, deferred storage or replay after expiry. Within that original window a later accepted poll can deliver if competition clears and all existing claim predicates still pass. Such a priority-held event uses its original transition-time payload and score, which can be older than the current score used to recheck eligibility; its payload is not refreshed when competition clears.

Raw conditions/transitions, trigger IDs, ranked-upset/final/kickoff rules, activation, expectation retention, atomic claim checks, subscriptions, history, origins and VAPID remain unchanged. Duke notifications remain hard-off. Regression coverage: `tests/alert-competition.test.mjs` plus the existing preferences, claim, activation and transport suites. These tests do not prove phone delivery.
