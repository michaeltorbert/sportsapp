...
export function GuideTimeline({ board, mode, now, followToday }: { board: Scoreboard; mode: GuideMode; now: number; followToday: boolean }) {
  ...
  return <>
    <div className="guide-canvas-toolbar"><span data-testid="guide-count">{model.games.length} {model.games.length === 1 ? "game" : "games"} listed{mode === "watch" ? ` · ${model.allCount} on this day` : ""}</span>{model.start !== null && <button className="guide-jump" onClick={() => { const el = scroller.current; if (el) { el.scrollLeft = now >= model.start! && now <= model.end! ? Math.max(0, coordinate(now, model.start!, SCALE) - 50) : 0; saveAnchor(); } }}>{now >= model.start && now <= model.end! ? "Now" : "Start"}</button>}</div>
      ...
      {rows.length > 0 && <><p id="guide-pan-help" className="guide-pan-help">Swipe to explore · Tab to games, arrow keys to scroll</p><div ref={scroller} className="guide-viewport" role="region" aria-label="Network and time schedule" aria-describedby="guide-pan-help guide-estimates" tabIndex={0} onScroll={saveAnchor}>
        <div className="guide-canvas" style={{ width: width + LABEL_WIDTH }}>
          ...
          {rows.map(lane => <section key={lane.key} className="guide-network" data-network={lane.key} style={{ height: lane.tracks * LANE_HEIGHT }} aria-label={lane.name}>
            <h2 className="guide-network-name">{lane.name}</h2><div className="guide-track" style={{ width }}>
