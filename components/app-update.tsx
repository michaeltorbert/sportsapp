...
export function AppUpdateNotice({ update }: { update: AppUpdate }) {
  return <><div role="status" aria-live="polite" className={update.target ? "app-update" : "sr-only"}>
    {update.target ? <><span>An app update is available.</span><div className="app-update-actions"><button onClick={update.refresh} aria-disabled={update.refreshing}>Refresh app</button><button onClick={update.dismiss} aria-disabled={update.refreshing}>Later</button></div></> : update.status}
